...
这就完成了数据的降维.
\subsection{降维的非线性方法}
\subsubsection{非线性潜变量模型与自编码器}
线性的降维方法实际上是通过线性映射$F_1$将高维的数据$\vec{x}$映射到低维的数据$\vec{z}$,并通过另一个映射$F_2$将其近似地还原为原来的$\vec{x}$.\\
\indent 由此,可以建立一种神经网络,它的前半部分为降维映射$F_1$,后半部分为还原映射$F_2$,其训练目标是尽可能还原输入数据$\vec{x}$.这样的神经网络被称作\tbf{自编码器(Autoencoder)}或\tbf{编码-解码器(Encoder-Decoder)}.\\
\indent 自编码器的一个应用就是AI换脸技术.假定有两个不同人的大量照片构成数据集$\mathcal{A}$和$\mathcal{B}$,将一个公用的编码器$E$与两个不同的解码器$D_A$和$D_B$各自组合后分别在两个数据集上训练,然后将$\mathcal{A}$的照片用$E-D_B$的组合进行处理即可得到换脸的结果.
\end{document}